...
while ( $in && ! feof( $in)) {
	...
	if ( ! isset( $h[ 'dyno']) || ! $h[ 'dyno'] || ! isset( $h[ 'bytes']) || ! $h[ 'bytes'] || ! isset( $h[ 'service']) || ! $h[ 'service'] || ! isset( $h[ 'status']) || $h[ 'status'] != 200) continue;
	$dyno = str_replace( 'web.', '', $h[ 'dyno']); $bytes = $h[ 'bytes'];
...
$out = fopen( '../state.csv', 'w'); fwrite( $out, "date,time,dyno,bytes,connect(ms),service(ms)\n"); 
